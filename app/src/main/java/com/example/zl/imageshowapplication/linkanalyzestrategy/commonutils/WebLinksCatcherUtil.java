...
//--------------------------------------NO NEEDED---------------------------------
//    /**
//    *  Get 方式请求服务器,针对 HTTPS 协议，返回 String
//    * @param url
//    *            请求地址
//    * @param charset
//    *            编码
...
//    }
    ...
    /**
     * 以 Byte 数组的形式获取 Get 操作结果，针对 HTTPS 协议
     ...
     * @throws IOException
     */
    private  static byte[] httpGetByte(String url) throws NoSuchAlgorithmException, KeyManagementException, IOException {
        SSLContext sc = SSLContext.getInstance("SSL");
        sc.init(null, new TrustManager[] { new TrustAnyTrustManager() }, new java.security.SecureRandom());
        URL console = new URL(url);
        HttpsURLConnection conn = (HttpsURLConnection) console.openConnection();
    ...
    /**
     * 以 Byte 数组的形式获取Get操作结果,针对 HTTP 协议
     * @param url 用Get方法访问的链接
     * @return
     * @throws IOException
     */
    ...
    /**
     * 获取链接内容并以UTF-8编码返回，针对 HTTPS 协议
     * @param srcurl
     * @return
     */
    public static String getHTTPSLinkSrc(String srcurl) {
        byte[] b = null;
        String src = null;
        try {
    ...
    /**
     * 获取链接内容并以UTF-8编码返回，针对 HTTP 协议
     * @param srcurl
     * @return
     */
    public static String getHTTPLinkSrc(String srcurl) {
        byte[] b = null;
        String src = null;
        try {

            b = httpGetByteWithoutSSL(srcurl);
            src = new String(b, "utf-8");
