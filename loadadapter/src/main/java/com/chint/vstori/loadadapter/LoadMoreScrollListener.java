...
public class LoadMoreScrollListener extends RecyclerView.OnScrollListener {
    ...
    public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
        ...
        if(manager instanceof LinearLayoutManager){
            ...
            Log.v("dyp","lastVisibleItemPosition:"+lastVisibleItemPosition+",adapter.getItemCount()-1 :"+ (adapter.getItemCount()-1) +",hasMore:"+adapter.hasMore + ",listSize: " + adapter.list.size() + ",loadState: " + adapter.loadState);

            if(adapter.hasMore && adapter.getItemCount() > adapter.getPageCount() && adapter.getItemCount()-1 == lastVisibleItemPosition){
                if(!adapter.isLoading()){
                    adapter.LoadingMore();
                    Log.i("AnalyzeCore","ScrollingLoading...");
                }
